...
public class TableListDT : MonoBehaviour {
	...
	public Sprite table_S4;
	public Sprite table_S5;
	public Sprite table_S9;
	...
	void Start ()
	{
		...
		gridview.spacing = new Vector2 (20, deltaHeight / 2);
	}
	...
	public void addTable (List<TableGame> tableList)
	{
		...
		Debug.Log (numberScreen);
		screenGame = new int[numberScreen,6];
		for(int i=0;i<numberScreen-1;i++){
			Debug.Log ("man "+i);
			for(int j=0;j<6;j++){
				screenGame[i,j] = j+i*6;
				Debug.Log("ban"+ screenGame[i,j]);
			}
		}
		Debug.Log ("man "+(numberScreen-1));
		for(int j=0;j<6;j++){
			screenGame[numberScreen-1,j] = tableList.Count-j;
			Debug.Log("ban"+ screenGame[numberScreen-1,j]);
		}
		for (int i = 0; i < tableList.Count; i++) {
			AddNewItem (tableList [i], i);
		}
//		for(int i=0; i<numberScreen-1;i++){
//			AddNewItem2(tableList[i],i);
//		}
	}
	void AddNewItem2(TableGame tableGame, int index){
	...
	void AddNewItem (TableGame tableGame, int index)
	{
		...
		for (int i=0; i<pl.Length; i++) {
			...
			rect.GetComponent<Image>().sprite = table_S9;
		}
		...
		if (GameApplication.cubeia.gameId == GameType.XOCDIA) {
			...
			if (max < 5) {
				for (int i=4; i<9; i++) {
					pl [i].GetComponent<Image> ().enabled = false;
					rect.GetComponent<Image>().sprite = table_S4;
				}
			}
			if (max == 5) {
				for (int i=5; i<9; i++) {
					pl [i].GetComponent<Image> ().enabled = false;
					rect.GetComponent<Image>().sprite = table_S5;
				}
			}
