...
public class ScreenList : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler {
		...
		for(int j=0;j<tableList.Count;j++){
			for(int i=0;i<6;i++){
				if(i==j){
					...
					if(tg.disable){
						rect.GetChild(i).GetComponent<Image>().color = new Color32(162,162,162,255);
					}
				}
			}
		}
